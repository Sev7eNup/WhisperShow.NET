...
public class OpenAiClientFactoryTests
{
    ...
    {
        var options = new VoxwrightOptions { OpenAI = { ApiKey = TestApiKey } };
        var monitor = new TestOptionsMonitor<VoxwrightOptions>(options);
        var factory = new OpenAiClientFactory(monitor);

        var first = factory.GetClient();

        var updated = new VoxwrightOptions { OpenAI = { ApiKey = TestApiKey + "-changed" } };
    ...
    public void GetClient_CreatesNewClient_WhenEndpointChanges()
    {
        ...
        var updated = new VoxwrightOptions
        {

...
/// the "Voxwright" section, and writes the result atomically (via <see cref="AtomicFileHelper"/>:
...
public class SettingsPersistenceService : ISettingsPersistenceService, IDisposable
{
    ...
    /// <summary>
    /// Schedules a setting change by providing a mutator that modifies the "Voxwright"
    ...
    /// </summary>
    /// <param name="mutator">
    /// A function that receives the "Voxwright" JSON section and modifies it in place.
    ...
    private async Task FlushCoreAsync()
    {
        ...
        try
        {
            ...
            var section = doc["Voxwright"];
            if (section is null)
            {
                _logger.LogError("appsettings.json is missing the 'Voxwright' section — skipping save");
                return;
            }
